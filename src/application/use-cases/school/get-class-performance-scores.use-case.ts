...
interface GetClassPerformanceScoreUseCaseInput {
  data: {
    schoolId?: string;
    gradeId: number;
    sectionId: number;
...
export class GetClassPerformanceScoreUseCase {
  constructor(private readonly dependencies: { businessRepo: BusinessRepository }) {}

  async execute(input: GetClassPerformanceScoreUseCaseInput) {
    ...
    const scores = await businessRepo.getSectionPerformanceScores(query);
    return scores;
  }
}
