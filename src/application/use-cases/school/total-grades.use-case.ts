...
interface TotalGradesUseCaseInput {
  data: {
    name?: string;
    schoolId?: string;
  };
...
export class TotalGradesUseCase {
  constructor(private readonly dependencies: { schoolRepo: SchoolRepository }) {}

  async execute(input: TotalGradesUseCaseInput) {
    ...
    return await schoolRepo.countGrades(payload);
  }
}
